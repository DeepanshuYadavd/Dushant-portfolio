...
export default function VideoCard({ project }: VideoCardProps) {
  ...
  return (
    <div
      ...
      onClick={() => !isReel && setIsPlaying(true)}
    >
      {isReel ? (
          ...
          {!isPlaying ? (
            <>
              ...
              <div className="!absolute !bottom-0 !left-0 !w-full !p-6 !translate-y-4 group-hover:!translate-y-0 !opacity-0 group-hover:!opacity-100 !transition-all !duration-500">
                <h3 className="!text-white !font-bold !text-base !tracking-tight">{project.title}</h3>
                <span className="!text-[9px] !uppercase !tracking-[0.3em] !text-neutral-400 !font-medium">{project.category}</span>
              </div>
